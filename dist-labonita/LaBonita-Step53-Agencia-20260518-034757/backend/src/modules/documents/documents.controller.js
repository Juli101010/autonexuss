...
exports.emitOfficialPdf = async (req, res) => {
  try {
    ...
    const totals = await db.get(
      `SELECT
        COALESCE(SUM(CASE WHEN direction='IN' THEN amount ELSE 0 END),0) AS ledger_in,
        COALESCE(SUM(CASE WHEN direction='OUT' THEN amount ELSE 0 END),0) AS ledger_out
       FROM ledger_entries WHERE petition_id = ?`,
      [petitionId]
    );
    const expenses = await db.get(`SELECT COALESCE(SUM(amount),0) AS expenses_total FROM expenses WHERE petition_id = ?`, [petitionId]);
    const commissions = await db.get(`SELECT COALESCE(SUM(amount),0) AS commissions_total FROM commissions WHERE petition_id = ?`, [petitionId]);
