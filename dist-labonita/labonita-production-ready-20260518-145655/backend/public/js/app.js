...
/**
  ...
  function renderNavForRole() {
    ...
    const links = IS_ADMIN
      ? [
          { r: 'dashboard', t: 'ðŸ  Dashboard', badge: null },
          { r: 'operativo', t: 'ðŸ§­ Operativo 2026', badge: null },
          { r: 'mapa', t: 'ðŸ—ºï¸ Mapa de Peticiones', badge: 'badgeValidation', cls: 'badge violet' },
          { r: 'peticiones', t: 'ðŸ“„ Peticiones', badge: 'badgePeticiones', cls: 'badge' },
          { r: 'facturacion', t: 'ðŸ§¾ FacturaciÃ³n', badge: null },
          { r: 'controles', t: 'ðŸ“Š Controles', badge: null },
          { r: 'avisos', t: 'ðŸ”” Avisos', badge: 'badgeAvisos', cls: 'badge red' },
          { r: 'delsol', t: 'ðŸ”Œ DelSol Sync', badge: 'badgeDelSol', cls: 'badge' },
          { r: 'citas', t: 'ðŸ“… Citas online', badge: null },
        ]
      : [
          { r: 'dashboard', t: 'ðŸ  Inicio', badge: null },
          { r: 'peticiones', t: 'ðŸ“„ Mis peticiones', badge: null },
          { r: 'mensajes', t: 'ðŸ’¬ Mensajes', badge: null },
          { r: 'perfil', t: 'ðŸ‘¤ Perfil', badge: null },
          { r: 'avisos', t: 'ðŸ”” Inbox', badge: 'badgeAvisos', cls: 'badge red' },
          { r: 'citas', t: 'ðŸ“… Citas online', badge: null },
        ];

    const iconByRoute = {
      dashboard: 'fa-solid fa-house',
      operativo: 'fa-solid fa-compass-drafting',
      mapa: 'fa-solid fa-map',
      peticiones: 'fa-solid fa-file-lines',
      facturacion: 'fa-solid fa-receipt',
      controles: 'fa-solid fa-chart-column',
      mensajes: 'fa-solid fa-comments',
      perfil: 'fa-solid fa-user',
      avisos: 'fa-solid fa-bell',
      delsol: 'fa-solid fa-plug-circle-check',
      citas: 'fa-solid fa-calendar-days',
    };

    nav.innerHTML = links
      .map((l, i) => {
        const badge = l.badge
          ? `<span class="${l.cls || 'badge'}" id="${l.badge}">${l.badge === 'badgeDelSol' ? 'Cargando' : '0'}</span>`
          : `<span class="nav-meta"></span>`;
        const active = i === 0 ? ' class="active"' : '';
        const cleaned = String(l.t || '')
          .replace(/^[^A-Za-z0-9ÃÃ‰ÃÃ“ÃšÃœÃ‘Ã¡Ã©Ã­Ã³ÃºÃ¼Ã±]+/, '')
          .trim();
        const icon = iconByRoute[l.r] || 'fa-solid fa-circle';
        return `<a href="#/${l.r}" data-route="${l.r}"${active}><span class="left"><i class="${icon}"></i><span>${cleaned}</span></span>${badge}</a>`;
      })
      .join('');
  }
  
  function renderNavForRoleClean() {
    const nav = document.getElementById('nav');
    if (!nav) return;
    const links = IS_ADMIN
      ? [
          { r: 'dashboard', label: 'Dashboard', icon: 'fa-house' },
          { r: 'operativo', label: 'Operativo 2026', icon: 'fa-compass' },
          { r: 'mapa', label: 'Mapa de Peticiones', icon: 'fa-map', badge: 'badgeValidation', cls: 'badge violet' },
          { r: 'peticiones', label: 'Peticiones', icon: 'fa-file-lines', badge: 'badgePeticiones', cls: 'badge' },
          { r: 'facturacion', label: 'Facturacion', icon: 'fa-receipt' },
          { r: 'controles', label: 'Controles', icon: 'fa-chart-column' },
          { r: 'avisos', label: 'Avisos', icon: 'fa-bell', badge: 'badgeAvisos', cls: 'badge red' },
          { r: 'delsol', label: 'DelSol Sync', icon: 'fa-plug-circle-check', badge: 'badgeDelSol', cls: 'badge' },
          { r: 'citas', label: 'Citas online', icon: 'fa-calendar-days' },
        ]
      : [
          { r: 'dashboard', label: 'Inicio', icon: 'fa-house' },
          { r: 'peticiones', label: 'Mis peticiones', icon: 'fa-file-lines' },
          { r: 'mensajes', label: 'Mensajes', icon: 'fa-comments' },
          { r: 'perfil', label: 'Perfil', icon: 'fa-user' },
          { r: 'avisos', label: 'Inbox', icon: 'fa-bell', badge: 'badgeAvisos', cls: 'badge red' },
          { r: 'citas', label: 'Citas online', icon: 'fa-calendar-days' },
        ];
    ...
    nav.innerHTML = links
      .map((l, i) => {
        const badge = l.badge
          ...
          : '<span class="nav-meta"></span>';
        const active = i === 0 ? ' class="active"' : '';
        return `<a href="#/${l.r}" data-route="${l.r}"${active}><span class="left"><i class="fa-solid ${l.icon}"></i><span>${l.label}</span></span>${badge}</a>`;
      })
      .join('');
  }

  function showPanel(panelId) {
    ['panelDashboard','panelFrame','panelMap','panelPeticiones','panelFacturacion','panelAdminWizard','panelControlesNative','panelAvisos','panelDelSol','panelCitas'].forEach((id) => {
  ...
  function setFrameLayout(enabled) {
    ...
    const available = Math.max(420, Math.floor(viewportH - topbarRect.bottom - 18));
  ...
  function openFrame(url) {
    ...
    frame.src = url;
    setFrameLayout(true);
    requestAnimationFrame(() => setFrameLayout(true));
    setTimeout(() => setFrameLayout(true), 120);
    setTimeout(() => setFrameLayout(true), 360);
  }
  ...
  function routeTo(route) {
    if (!IS_ADMIN) {
      ...
      if (route === 'peticiones') {
        ...
        openFrame('/artist-operativo.html');
        return;
      }
      if (route === 'mensajes') {
        setActive('mensajes');
        setTitle('Mensajes', 'Conversaciones por peticiÃ³n');
        showPanel('panelFrame');
        openFrame('/artist-mensajes.html');
        return;
      }
      if (route === 'perfil') {
        setActive('perfil');
      ...
      if (route === 'citas') {
        setActive('citas');
        setTitle('Citas online', 'Agenda operativa');
    ...
    if (route === 'dashboard') {
      setActive('dashboard');
      setTitle('Dashboard', 'Vista operativa');
      showPanel('panelDashboard');
      setFrameLayout(false);
      return;
    }
    if (route === 'operativo') {
      setActive('operativo');
      setTitle('Operativo 2026', 'Mapa de Peticiones + facturacion interna + DELSOL live/cola');
      showPanel('panelMap');
      setFrameLayout(false);
      wireMapControls();
    ...
    if (route === 'controles') {
      ...
      showPanel('panelControlesNative');
      setFrameLayout(false);
      setTimeout(() => {
        wireControlsNative();
        loadControlsNative().catch(() => {});
      }, 0);
      return;
    }
    ...
    if (route === 'facturacion') {
      ...
      setTitle('FacturaciÃ³n', 'Editar datos fiscales y conciliar DelSol');
    ...
    if (route === 'delsol') {
      setActive('delsol');
      setTitle('DelSol Sync', 'Estado real de conexion y cola');
      showPanel('panelDelSol');
      setFrameLayout(false);
      setTimeout(() => {
        wireDelsolPanel();
        loadDelsolPanel().catch(() => {});
      }, 0);
      return;
    }
    ...
    if (route === 'peticiones') {
      setActive('peticiones');
      setTitle('Peticiones', 'Gestion completa + validacion + workflow');
      showPanel('panelPeticiones');
      setFrameLayout(false);
      setTimeout(() => {
        wirePetitionsUI();
        loadPetitionsNative().catch(() => {});
      }, 0);
      return;
    }
    ...
    if (route === 'peticiones' && __openPetitionId) {
      openFrame(`${base}?petition_id=${encodeURIComponent(__openPetitionId)}`);
      __openPetitionId = null;
    } else {
      openFrame(base);
    }
    if (route === 'peticiones') setTitle('Peticiones', 'GestiÃ³n completa + validaciÃ³n + workflow');
  ...
  async function refreshBadges() {
    ...
    try {
      const ds = await api('/admin/delsol/status');
      const delsolCard = document.querySelector('[data-goto="delsol"]');
      const value = document.getElementById('kpiDelsolStatus') || delsolCard?.querySelector('.value');
      const hint = document.getElementById('kpiDelsolHint') || delsolCard?.querySelector('.hint');
      const pending = Number(ds.queue?.pending || 0) + Number(ds.queue?.error || 0) + Number(ds.queue?.blocked_permission || 0);
      if (value) value.textContent = ds.mode === 'live' && ds.write_enabled ? 'Live' : ds.mode || 'Mock';
      if (hint) hint.textContent = pending ? `${pending} items en cola` : 'Sincronizado / sin pendientes';
      const badge = document.getElementById('badgeDelSol');
      if (badge) badge.textContent = pending ? String(pending) : (ds.mode === 'live' ? 'Live' : 'Mock');
    } catch {
      const delsolCard = document.querySelector('[data-goto="delsol"]');
      const value = document.getElementById('kpiDelsolStatus') || delsolCard?.querySelector('.value');
      const hint = document.getElementById('kpiDelsolHint') || delsolCard?.querySelector('.hint');
      if (value) value.textContent = 'Error';
      if (hint) hint.textContent = 'No se pudo leer estado';
    }
  }

  function wireDashboardLinks() {
    document.querySelectorAll('.app-kpi-link[data-goto]').forEach((card) => {
  ...
  window.addEventListener('resize', () => setFrameLayout(!document.getElementById('panelFrame')?.classList.contains('hidden')));
  
  (async function boot() {
    const frame = document.getElementById('frame2');
    if (frame) {
      frame.addEventListener('load', () => setFrameLayout(!document.getElementById('panelFrame')?.classList.contains('hidden')));
    }
    renderNavForRoleClean();
    wireDashboardLinks();
    wireDrawer();
    wireSearch();
    wireLogout();
  ...
  function stepMeta() {
    return [
      { key: 'petition', label: '1) PeticiÃ³n', hint: 'Datos base', icon: 'fa-solid fa-file-circle-plus' },
      { key: 'validation', label: '2) ValidaciÃ³n', hint: 'Admin valida', icon: 'fa-solid fa-shield-check' },
      ...
      { key: 'facturacion', label: '5) FacturaciÃ³n', hint: 'Factura emitida', icon: 'fa-solid fa-receipt' },
      { key: 'calculos', label: '6) CÃ¡lculos', hint: 'Gastos/ComisiÃ³n/Ledger', icon: 'fa-solid fa-calculator' },
  ...
  function rowLabel(r) {
    const ref = r.reference ? ` Â· ${r.reference}` : '';
    return `#${r.id} Â· ${r.type} Â· ${r.status}${ref}`;
  }
  ...
  function renderMapSelect() {
    ...
    optAll.value = '';
    optAll.textContent = 'Seleccionar peticiÃ³nâ€¦';
  ...
  function renderMapActions() {
    ...
    if (!row) {
      wrap.innerHTML = '<span class="small">Selecciona una peticiÃ³n para ver acciones.</span>';
      return;
    }
    ...
    btnAssignMe.textContent = 'Asignar a mÃ­';
    ...
    info.textContent = row.assigned_admin_id ? `Asignada a admin#${row.assigned_admin_id}` : 'Sin asignaciÃ³n';
  ...
  function renderMapSteps() {
    ...
    if (!row) {
      hint.textContent = 'Selecciona una peticiÃ³n para ver el progreso. TambiÃ©n puedes filtrar por paso.';
    } else {
      const bal = Number(row.balance_to_zero || 0);
      hint.textContent = `Balance a cero: â‚¬${bal.toFixed(2)} Â· Checklist faltantes: ${row.checklist_missing}`;
    }
    ...
    stepMeta().forEach((s) => {
      ...
      const missText = miss.length ? miss.slice(0,3).join(' Â· ') : '';
      ...
      div.innerHTML = `
        ...
        <div class="label" style="margin-top:6px;">${row ? (ok ? 'âœ… OK' : 'â³ Pendiente') : 'â€”'}</div>
        ${row ? `<div class="label" style="margin-top:6px; opacity:.85;">${miss.length ? `Falta: ${missText}${miss.length>3?'â€¦':''}` : ''}</div>` : ''}
      `;
  ...
  function renderMapTable() {
    ...
    rows.slice(0, 200).forEach((r) => {
      ...
      tr.innerHTML = `
        ...
        <td>${r.docs_count}</td>
        <td>â‚¬${Number(r.flags?.ledgerIn || 0).toFixed(2)}</td>
        <td>â‚¬${Number(r.flags?.ledgerOut || 0).toFixed(2)}</td>
        <td>â‚¬${Number(r.balance_to_zero || 0).toFixed(2)}</td>
      `;
      tr.onclick = () => {
  ...
  async function selectPetitionNative(id) {
    ...
    wrap.innerHTML = '<div class="small">Cargandoâ€¦</div>';
    ...
    try {
      ...
      wrap.innerHTML = `
        <h3 style="margin:0 0 8px 0;">#${id} Â· ${p.type} Â· ${p.status}</h3>
        <div class="small">Referencia: <strong>${row.reference || ''}</strong></div>
        <div class="small">Checklist faltantes: <strong>${row.checklist_missing}</strong> Â· Docs: <strong>${row.docs_count}</strong></div>
        <div id="petMissingBox" style="margin-top:10px;"></div>
        <div class="small">Ledger IN: <strong>â‚¬${Number(row.flags?.ledgerIn || 0).toFixed(2)}</strong> Â· OUT: <strong>â‚¬${Number(row.flags?.ledgerOut || 0).toFixed(2)}</strong></div>
        <div class="small">Balance a cero: <strong>â‚¬${bal.toFixed(2)}</strong></div>
  
        <div style="margin-top:10px; display:flex; gap:8px; flex-wrap:wrap;">
          <button class="btn" type="button" onclick="window.open('/admin-controles.html?petition_id=${id}&focus=ledger','_blank')">Ir a Controles â†’ Ledger</button>
          <button class="btn" type="button" onclick="window.open('/admin-controles.html?petition_id=${id}&focus=control-total','_blank')">Ir a Controles â†’ Control Total</button>
        ...
        <div style="display:flex; gap:8px; flex-wrap:wrap; align-items:center;">
          ...
          <button id="btnPetEmitNomina" class="btn btn-primary" type="button">Emitir PDF NÃ³mina</button>
        ...
        <div style="display:flex; gap:8px; flex-wrap:wrap; align-items:center;">
          ...
          <input id="petMsgBody" class="btn" placeholder="Escribirâ€¦" style="flex:1; min-width:260px;" />
          <button id="btnPetSendMsg" class="btn" type="button">Enviar</button>
        </div>
        <div id="petMessages" class="small" style="margin-top:8px;">â€”</div>
  
        <h4 style="margin:14px 0 6px 0;">Timeline</h4>
        <div id="petTimeline" class="small">Cargando timelineâ€¦</div>
  
        <h4 style="margin:14px 0 6px 0;">QuÃ© falta por paso</h4>
        ...
        ${stepMeta().map((s) => {
          ...
          const badge = st === 'OK' ? 'âœ…' : (st === 'NA' ? 'â€”' : (st === 'BLOCK' ? 'â›”' : 'â³'));
          return `<div class="small"><strong>${badge} ${s.label}</strong>: ${miss.length ? miss.map((x)=>String(x)).join(' Â· ') : ''}</div>`;
        }).join('')}
      `;
  ...
  async function loadTimelineNative(petitionId) {
    ...
    if (!el) return;
    el.textContent = 'Cargando timelineâ€¦';
    try {
      ...
      el.innerHTML = rows.slice(0, 40).map((r) => {
        ...
        return `<div style="padding:6px 0; border-bottom:1px solid rgba(0,0,0,.06);">
          <strong>${r.event_type}</strong> Â· ${when} Â· ${who}<br/>
  ...
  async function wirePetitionActionsNative(petitionId, type, currentStatus) {
    ...
    document.getElementById('btnPetReject').onclick = async () => {
      if (!confirm('Rechazar peticiÃ³n?')) return;
      await api(`/admin/petitions/${petitionId}/status`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
    ...
    emitNomina.onclick = async () => {
      const out = document.getElementById('petDocResult');
      out.textContent = 'Emitiendo NÃ“MINA...';
      await api(`/admin/petitions/${petitionId}/emit/NOMINA`, { method: 'POST' });
      out.textContent = 'OK. NÃ³mina PDF publicada.';
    };
  ...
  async function loadPetitionMessagesNative(petitionId) {
    ...
    try {
      ...
      box.innerHTML = rows.slice(-30).map((m) => {
        ...
        const kind = m.kind === 'AVISO' ? 'ðŸ”” AVISO' : 'ðŸ’¬';
        return `<div style="padding:6px 0; border-bottom:1px solid rgba(0,0,0,.06);">
          <strong>${kind}</strong> Â· ${who} Â· ${m.created_at}<br/>
  ...
  function renderInboxList() {
    ...
    if (inboxTab === 'ann') {
      ...
      el.innerHTML = rows.length ? rows.map((a) => {
        ...
        return `<div ${sel} style="padding:8px; border-bottom:1px solid rgba(0,0,0,.06); cursor:pointer;" data-ann="${a.id}">
          <div><strong>ðŸ“£ ${(a.title || '').slice(0, 90)}</strong></div>
          <div class="small">${a.scope}${a.scope_value ? ` Â· ${a.scope_value}` : ''} Â· ${a.created_at || ''}</div>
        </div>`;
    ...
    if (inboxTab === 'threads') {
      ...
      el.innerHTML = rows.length ? rows.map((t) => {
        const unread = Number(t.unread_count || 0) ? `ðŸ”´ ${t.unread_count} ` : '';
        ...
        return `<div ${sel} style="padding:8px; border-bottom:1px solid rgba(0,0,0,.06); cursor:pointer;" data-thread="${t.petition_id}">
          <div><strong>${unread}PeticiÃ³n #${t.petition_id} Â· ${t.type}</strong></div>
          <div class="small">${(t.last_body || '').slice(0, 120)}</div>
          <div class="small">${t.status} Â· ${t.last_at || ''}</div>
        </div>`;
    ...
    if (inboxTab === 'groups') {
      ...
      el.innerHTML = `
        <div style="display:flex; gap:8px; flex-wrap:wrap; align-items:center; margin-bottom:10px;">
          ...
          <input id="grpDesc" class="btn" placeholder="DescripciÃ³n" style="flex:1; min-width:220px;" />
        ...
        ${rows.length ? rows.map((g) => {
          ...
          return `<div ${sel} style="padding:8px; border-bottom:1px solid rgba(0,0,0,.06); cursor:pointer;" data-group="${g.id}">
            <div><strong>ðŸ‘¥ ${g.name}</strong></div>
  ...
  async function renderInboxDetail() {
    ...
    if (!inboxSelected) {
      el.textContent = 'SeleccionÃ¡ un item.';
      return;
    }
    ...
    if (inboxSelected.type === 'notif') {
      ...
      el.innerHTML = `
        <h3 style="margin:0 0 8px 0;">NotificaciÃ³n</h3>
        <div class="small">${n.created_at || ''}</div>
        <p>${(n.message || '').replace(/</g,'&lt;')}</p>
        <div style="display:flex; gap:8px; flex-wrap:wrap; margin-top:10px;">
          <button class="btn" type="button" id="btnNotifRead">${n.read ? 'LeÃ­da' : 'Marcar leÃ­da'}</button>
    ...
    if (inboxSelected.type === 'ann') {
      ...
      el.innerHTML = `
        <h3 style="margin:0 0 8px 0;">ðŸ“£ ${String(a.title||'').replace(/</g,'&lt;')}</h3>
        <div class="small">${a.created_at || ''} Â· ${a.scope}${a.scope_value ? ` Â· ${a.scope_value}` : ''}</div>
    ...
    if (inboxSelected.type === 'thread') {
      ...
      try {
        ...
        el.innerHTML = `
          <h3 style="margin:0 0 8px 0;">Thread â€” PeticiÃ³n #${pid}</h3>
          ...
          <div id="inboxMsgs">
            ${rows.map((m) => `<div style="padding:6px 0; border-bottom:1px solid rgba(0,0,0,.06);">
              <strong>${m.kind === 'AVISO' ? 'ðŸ”” AVISO' : 'ðŸ’¬'}</strong> Â· ${m.from_role}#${m.from_user_id} Â· ${m.created_at}<br/>
              ${String(m.body).replace(/</g,'&lt;')}
    ...
    if (inboxSelected.type === 'group') {
      ...
      try {
        ...
        el.innerHTML = `
          ...
          <div style="display:flex; gap:8px; flex-wrap:wrap; margin-bottom:8px;">
            <input id="grpUserEmail" class="btn" placeholder="Buscar emailâ€¦" style="flex:1; min-width:220px;" /><div id="grpUserResults" class="small" style="width:100%;"></div>
          ...
          <div>
            ${rows.length ? rows.map((u) => `<div style="padding:6px 0; border-bottom:1px solid rgba(0,0,0,.06);">
              <strong>${u.email}</strong> Â· ${u.role} Â· id=${u.id}
      ...
      try {
        ...
        box.innerHTML = rows.length ? rows.map((u) => 
          `<div style="padding:6px 0; border-bottom:1px solid rgba(0,0,0,.06); display:flex; justify-content:space-between; gap:8px;">
             <span><strong>${u.email}</strong> Â· ${u.role} Â· id=${u.id}</span>
  ...
  function wireControlsNative() {
    const refresh = document.getElementById('btnCtrlRefresh');
    if (refresh && !refresh.dataset.wired) {
      refresh.dataset.wired = '1';
      refresh.onclick = () => loadControlsNative();
    }
    const full = document.getElementById('btnCtrlOpenFull');
    if (full && !full.dataset.wired) {
      full.dataset.wired = '1';
      full.onclick = () => window.open('/admin-controles.html', '_blank');
    }
    const input = document.getElementById('ctrlPetitionId');
    if (input && !input.dataset.wired) {
      input.dataset.wired = '1';
      input.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') loadControlsNative();
      });
    }
  }

  async function loadControlsNative() {
    const proposalsBody = document.getElementById('ctrlProposals');
    const ledgerBody = document.getElementById('ctrlLedger');
    const exportsBox = document.getElementById('ctrlExports');
    if (proposalsBody) proposalsBody.innerHTML = '<tr><td colspan="6">Cargando...</td></tr>';
    if (ledgerBody) ledgerBody.innerHTML = '<tr><td colspan="6">Cargando...</td></tr>';

    const petitionId = Number(document.getElementById('ctrlPetitionId')?.value || 0);
    const [proposals, ledger] = await Promise.all([
      api('/admin/payout-proposals?status=PENDING').catch(() => ({ rows: [] })),
      api(`/admin/ledger${petitionId ? `?petition_id=${encodeURIComponent(petitionId)}` : ''}`).catch(() => ({ rows: [] })),
    ]);

    if (proposalsBody) {
      const rows = proposals.rows || [];
      proposalsBody.innerHTML = rows.length ? rows.slice(0, 40).map((r) => `
        <tr>
          <td>#${r.id}</td><td>#${r.petition_id}</td><td>${esc(r.occurred_at || '')}</td>
          <td>EUR ${Number(r.proposed_amount_eur || 0).toFixed(2)}</td>
          <td><button class="btn" data-proposal-act="approve" data-id="${r.id}" type="button">Aprobar</button></td>
          <td><button class="btn" data-proposal-act="reject" data-id="${r.id}" type="button">Rechazar</button></td>
        </tr>
      `).join('') : '<tr><td colspan="6">Sin propuestas pendientes.</td></tr>';
      proposalsBody.querySelectorAll('[data-proposal-act]').forEach((btn) => {
        btn.onclick = async () => {
          await api(`/admin/payout-proposals/${btn.dataset.id}/${btn.dataset.proposalAct}`, { method: 'POST' });
          await loadControlsNative();
        };
      });
    }
    ...
    if (ledgerBody) {
      ...
      ledgerBody.innerHTML = rows.length ? rows.slice(0, 80).map((r) => `
        <tr><td>#${r.id}</td><td>#${r.petition_id}</td><td>${esc(r.direction)}</td><td>${esc(r.category)}</td><td>EUR ${Number(r.amount_eur || 0).toFixed(2)}</td><td>${esc(r.occurred_at || '')}</td></tr>
      `).join('') : '<tr><td colspan="6">Sin movimientos de ledger.</td></tr>';
    }

    if (exportsBox && !exportsBox.dataset.ready) {
      exportsBox.dataset.ready = '1';
      const exports = [
        ['Control total', '/api/admin/ops/export/control-total.csv'],
        ['Altas/Bajas', '/api/admin/ops/export/altas-bajas.csv'],
        ['Gastos', '/api/admin/ops/export/gastos.csv'],
        ['Liquidaciones', '/api/admin/ops/export/liquidaciones.csv'],
        ['Ledger', '/api/admin/controls/ledger?download=1&format=csv'],
      ];
      exportsBox.innerHTML = exports.map(([label, href]) => `<a class="btn" href="${href}">${label}</a>`).join('');
    }
  }

  function wireDelsolPanel() {
    ensureDelsolPanelShell();
    const refresh = document.getElementById('btnDelsolRefresh');
    if (refresh && !refresh.dataset.wired) {
      refresh.dataset.wired = '1';
      refresh.onclick = () => loadDelsolPanel();
    }
    const read = document.getElementById('btnDelsolRead');
    if (read && !read.dataset.wired) {
      read.dataset.wired = '1';
      read.onclick = async () => {
        const out = document.getElementById('dsOut');
        if (out) out.textContent = 'Probando lectura...';
        try {
          const r = await api('/admin/delsol/test-read', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
          if (out) out.textContent = `Lectura OK (${r.fallback || r.table || 'DELSOL'}).`;
        } catch (e) {
          if (out) out.textContent = `Lectura con error: ${e.message}`;
        }
        await loadDelsolPanel();
      };
    }
    const process = document.getElementById('btnDelsolProcess');
    if (process && !process.dataset.wired) {
      process.dataset.wired = '1';
      process.onclick = async () => {
        const out = document.getElementById('dsOut');
        if (out) out.textContent = 'Procesando cola...';
        try {
          const r = await api('/admin/delsol/queue/process', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ limit: 20 }) });
          if (out) out.textContent = `Cola procesada: ${r.processed || 0} items.`;
        } catch (e) {
          if (out) out.textContent = `Proceso con error: ${e.message}`;
        }
        await loadDelsolPanel();
        await refreshBadges();
      };
  ...
  async function loadDelsolPanel() {
    ensureDelsolPanelShell();
    const [status, queue] = await Promise.all([
      api('/admin/delsol/status'),
      api('/admin/delsol/queue').catch(() => ({ rows: [] })),
    ]);
    const q = status.queue || {};
    const pending = Number(q.pending || 0) + Number(q.error || 0) + Number(q.blocked_permission || 0);
    const synced = Number(q.synced || 0);
    const set = (id, value) => { const el = document.getElementById(id); if (el) el.textContent = value; };
    set('dsMode', status.mode || '-');
    set('dsExercise', `Ejercicio ${status.ejercicio || '-'}`);
    set('dsWrite', status.write_enabled ? 'Activa' : 'Bloqueada');
    set('dsPending', String(pending));
    set('dsSynced', String(synced));
    const out = document.getElementById('dsOut');
    if (out && !out.textContent) out.textContent = status.note || '';
    const body = document.getElementById('dsQueue');
    if (body) {
      const rows = queue.rows || [];
      body.innerHTML = rows.length ? rows.slice(0, 80).map((r) => `
        <tr>
          <td>#${r.id}</td><td>${esc(r.entity_type)}</td><td>${r.petition_id ? `#${r.petition_id}` : ''}</td>
          <td>${esc(r.operation)}</td><td><span class="badge">${esc(r.status)}</span></td>
          <td>${esc(r.last_error || '')}</td>
          <td>${r.status === 'synced' ? '' : `<button class="btn" data-ds-process="${r.id}" type="button">Procesar</button>`}</td>
        </tr>
      `).join('') : '<tr><td colspan="7">Sin movimientos de cola.</td></tr>';
      body.querySelectorAll('[data-ds-process]').forEach((btn) => {
        btn.onclick = async () => {
          const out = document.getElementById('dsOut');
          if (out) out.textContent = `Procesando item #${btn.dataset.dsProcess}...`;
          await api(`/admin/delsol/queue/${btn.dataset.dsProcess}/process`, { method: 'POST' });
          if (out) out.textContent = `Item #${btn.dataset.dsProcess} procesado.`;
          await loadDelsolPanel();
          await refreshBadges();
        };
      });
  ...
  function ensureDelsolPanelShell() {
    const panel = document.getElementById('panelDelSol');
    ...
    panel.innerHTML = `
      <div class="app-section-title">
        <div>
          <h2>DelSol Sync</h2>
          <p>Estado real de conexion, lectura API, cola de facturacion y nomina.</p>
        </div>
        <div class="toolbar">
          <button class="btn" id="btnDelsolRefresh" type="button"><i class="fa-solid fa-rotate"></i> Refrescar</button>
          <button class="btn" id="btnDelsolRead" type="button"><i class="fa-solid fa-plug-circle-check"></i> Probar API</button>
          <button class="btn primary" id="btnDelsolProcess" type="button"><i class="fa-solid fa-cloud-arrow-up"></i> Procesar cola</button>
        </div>
      </div>
      <div class="kpi-grid">
        <div class="kpi"><i class="fa-solid fa-signal"></i><div>Modo</div><div class="value" id="dsMode">-</div><div class="hint" id="dsExercise">Ejercicio -</div></div>
        <div class="kpi"><i class="fa-solid fa-pen-to-square"></i><div>Escritura</div><div class="value" id="dsWrite">-</div><div class="hint">live solo desde servidor</div></div>
        <div class="kpi"><i class="fa-solid fa-hourglass-half"></i><div>Pendientes</div><div class="value" id="dsPending">0</div><div class="hint">errores incluidos</div></div>
        <div class="kpi"><i class="fa-solid fa-circle-check"></i><div>Sincronizados</div><div class="value" id="dsSynced">0</div><div class="hint">impactos confirmados</div></div>
      </div>
      <div class="card" style="padding:12px; margin-top:12px;">
        <div class="card-title">Cola DelSol</div>
        <p class="small">Si DelSol rechaza o se corta la conexion, la operacion queda guardada para reprocesar una por una o en lote.</p>
        <div class="small" id="dsOut" style="margin:8px 0;"></div>
        <div class="table-wrap">
          <table class="data-table">
            <thead><tr><th>ID</th><th>Entidad</th><th>Peticion</th><th>Operacion</th><th>Estado</th><th>Error</th><th>Accion</th></tr></thead>
            <tbody id="dsQueue"><tr><td colspan="7">Cargando...</td></tr></tbody>
          </table>
        </div>
      </div>
    `;
  ...
  function renderCitasPanel() {
    const panel = document.getElementById('panelCitas');
    if (!panel) return;
    panel.innerHTML = `
      <div class="app-section-title">
        <div>
          <h2>Citas online</h2>
          <p>Agenda controlada para reuniones, altas, validaciones y soporte de peticiones.</p>
        </div>
        <button class="btn" type="button" onclick="location.hash='#/avisos'"><i class="fa-solid fa-bell"></i> Ver avisos</button>
      </div>
      <div class="kpi-grid">
        <div class="kpi"><i class="fa-solid fa-calendar-check"></i><div>Proxima cita</div><div class="value">A definir</div><div class="hint">se confirma desde administracion</div></div>
        <div class="kpi"><i class="fa-solid fa-user-check"></i><div>Uso recomendado</div><div class="value">Alta socio</div><div class="hint">validacion documental</div></div>
        <div class="kpi"><i class="fa-solid fa-file-invoice"></i><div>Soporte</div><div class="value">Factura</div><div class="hint">datos fiscales y DelSol</div></div>
      </div>
      <div class="card" style="padding:12px; margin-top:12px;">
        <div class="card-title">Flujo de citas</div>
        <div class="timeline-list">
          <div class="timeline-item"><strong>1. Solicitud</strong><span>El usuario pide ayuda desde una peticion o mensaje.</span></div>
          <div class="timeline-item"><strong>2. Revision admin</strong><span>Administracion valida si falta documentacion o datos fiscales.</span></div>
          <div class="timeline-item"><strong>3. Resultado</strong><span>La cita termina con aviso, correccion pedida, alta validada o factura lista.</span></div>
        </div>
      </div>
    `;
  }
  ...
  function notifDeepLink(n) {
    ...
    // fallback: parse "[PeticiÃ³n #123]"
    const m = String(n.message||'').match(/PeticiÃ³n\s*#(\d+)/i);
  ...
  function renderPetMissing(id, chk) {
      ...
      <div class="card" style="padding:12px; margin-top:10px;">
        ...
        <div class="small" style="margin-top:8px;">
          ${missingFields.slice(0, 30).map((f) => {
            ...
            return `<div>â€¢ <strong>${esc(f.label)}</strong> <span class="small">(${esc(path)})</span> <button class="btn" style="margin-left:8px; padding:6px 10px;" type="button" onclick="${target}">Ir</button></div>`;
          }).join('')}
          ${missingFields.length > 30 ? `<div class="small">+${missingFields.length-30} mÃ¡sâ€¦</div>` : ''}
        </div>
      </div>` : '';
    ...
    const docsHtml = missingDocs.length ? `
      <div class="card" style="padding:12px; margin-top:10px;">
        ...
        <div class="small" style="margin-top:8px;">
          ${missingDocs.map((d) => `<div>â€¢ <strong>${esc(d.doc_type)}</strong></div>`).join('')}
        </div>
      </div>` : '';
    ...
    const rulesHtml = ruleErrors.length ? `
      <div class="card" style="padding:12px; margin-top:10px;">
        ...
        <div class="small" style="margin-top:8px;">
          ${ruleErrors.map((r) => `<div>â€¢ <strong>${esc(r.code)}</strong>: ${esc(r.message)}</div>`).join('')}
        </div>
      </div>` : '';
  ...
  async function renderInvoiceRecon(petitionId, type) {
    ...
    box.innerHTML = `
      <div class="card" style="padding:12px;">
        <div style="display:flex; justify-content:space-between; gap:10px; flex-wrap:wrap; align-items:center;">
          <div>
            <strong>FacturaciÃ³n Â· ConciliaciÃ³n DelSol</strong>
            <div class="small">La numeraciÃ³n oficial se asigna al conciliar con DelSol (auto o manual). Antes de eso, el PDF queda â€œsin nÃºmeroâ€.</div>
          </div>
        ...
        <div class="small" style="margin-top:10px; display:grid; grid-template-columns: 1fr 1fr; gap:10px;" id="invGrid">
          ...
          <div>
            <div class="small"><strong>NÃºmero</strong></div>
    ...
    document.getElementById('btnRecon').onclick = async () => {
      ...
      out.textContent = 'OK. ConciliaciÃ³n guardada.';
    };
  }
  ...
  async function loadFacturacion(petitionId) {
    ...
    if (!body) return;
    if (!petitionId) {
      body.innerHTML = '<div class="skeleton" style="height:22px; margin-bottom:10px;"></div><div class="skeleton" style="height:120px;"></div>';
      try {
        const data = await api('/admin/ops/petitions');
        const rows = (data.rows || []).filter((r) => {
          const status = String(r.status || '').toUpperCase();
          return status.includes('APPROVED') || status.includes('APROB') || status.includes('VALID') || status.includes('ALTA_BAJA') || status.includes('FACTURA') || status.includes('LIQUIDACION');
        }).slice(0, 12);
        body.innerHTML = `
          <div class="card" style="padding:12px;">
            <div class="card-title">Peticiones listas para facturar</div>
            <p class="small">La factura interna nace desde una peticion validada por administracion. Selecciona una para revisar datos fiscales, emitir PDF y dejar cola DelSol.</p>
            <div class="table-wrap">
              <table class="data-table">
                <thead><tr><th>ID</th><th>Tipo</th><th>Estado</th><th>Responsable</th><th>Total</th><th>Accion</th></tr></thead>
                <tbody>
                  ${rows.length ? rows.map((r) => `
                    <tr>
                      <td>#${r.id}</td><td>${esc(r.type || '')}</td><td><span class="badge ok">${esc(r.status || '')}</span></td>
                      <td>${esc(r.responsible_name || r.company || '')}</td><td>EUR ${Number(r.invoice_total_eur || r.amount_eur || 0).toFixed(2)}</td>
                      <td><button class="btn primary" type="button" onclick="location.hash='#/facturacion?petition_id=${r.id}'">Abrir</button></td>
                    </tr>
                  `).join('') : '<tr><td colspan="6">No hay peticiones aprobadas visibles. Ejecuta una prueba completa o valida una peticion desde Peticiones.</td></tr>'}
                </tbody>
              </table>
            </div>
          </div>
        ...
        body.innerHTML = `<div class="card" style="padding:12px;">No se pudo cargar facturacion: ${esc(e.message)}</div>`;
      }
      return;
    }
  
    body.innerHTML = `<div class="skeleton" style="height:22px; margin-bottom:10px;"></div><div class="skeleton" style="height:160px;"></div>`;
    const detail = await api(`/admin/ops/petitions/${petitionId}`);
    const p = detail.petition || {};
    const schema = await api(`/admin/petitions/${petitionId}/schema`);
    const chk = await api(`/admin/petitions/${petitionId}/checklist`);
  
    const fields = (schema.fields||[]).filter((f)=>String(f.path||'').includes('invoice') || String(f.path||'').startsWith('factura_') || ['client_name','client_tax_id','factura_direccion','factura_cp','factura_ciudad','factura_pais','factura_concepto','factura_total_sin_iva','iva_tipo'].includes(String(f.path||'')));
    const payload = p.data_json || p.data || {};
    const invMissing = (chk?.checklist?.ruleErrors||[]).map((r)=>`â€¢ ${esc(r.message)}`).join('<br/>');
      ...
      <div class="card" style="padding:12px;">
        <div class="card-header">
          <div>
            ...
            <div class="card-subtitle">Reglas por paÃ­s: ${invMissing ? '<span class="badge warn">Revisar</span>' : '<span class="badge ok">OK</span>'}</div>
          </div>
    ...
    document.getElementById('btnFactSave').onclick = async () => {
      ...
      await api(`/admin/ops/petitions/${petitionId}`, { method:'PUT', headers:{'Content-Type':'application/json'}, body: JSON.stringify(newPayload) });
      out.textContent = 'OK.';
    };
  
    document.getElementById('btnFactEmitFactura').onclick = async () => {
      const out = document.getElementById('factOut');
      out.textContent = 'Creando factura interna y cola DelSol...';
      const total = Number(newPayload.factura_total_sin_iva || newPayload.invoice?.amount_without_vat || newPayload.total_presupuestado || 0);
      const created = await api(`/admin/ops/petitions/${petitionId}/invoices`, {
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body: JSON.stringify({
          subtotal: total,
          concept: newPayload.factura_concepto || newPayload.invoice?.concept || newPayload.description || newPayload.reference || `Peticion #${petitionId}`,
          client_name: newPayload.client_name || newPayload.invoice?.razon_social,
          client_tax_id: newPayload.client_tax_id || newPayload.invoice?.tax_id,
          ejercicio: '2026'
        })
      });
      out.textContent = `OK. Factura interna ${created.internal_number || created.id} creada y enviada a cola DelSol.`;
      await refreshBadges();
    };
  }
  ...
  function adminWizardStepsForType(type, fields) {
    ...
    if (tU === 'ALTA_FACTURA') {
      return [
        ...
        mk('factura','FacturaciÃ³n','fa-solid fa-receipt',(f)=>invoicePrefix.some(p=>String(f.path||'').startsWith(p))),
    ...
    return [mk('factura','FacturaciÃ³n','fa-solid fa-receipt',(f)=>String(f.path||'').startsWith('payload.invoice'))].filter(s=>s.fields.length);
  }
  ...
  async function loadAdminWizard(petitionId) {
    ...
    if (!body) return;
    if (!petitionId) { body.innerHTML = 'SeleccionÃ¡ una peticiÃ³n.'; return; }
        ...
        <div class="card" style="padding:12px;">
          <div class="card-header">
            <div>
              ...
              <div class="card-subtitle">EdiciÃ³n limitada (whitelist). Workflow no se toca. ${(missing.length||missingDocs.length) ? `<span class=\"badge warn\">Faltan ${missing.length} campos Â· ${missingDocs.length} docs</span>` : `<span class=\"badge ok\">OK</span>`}</div>
            </div>
            <div style="display:flex; gap:8px; flex-wrap:wrap;">
              <button class="btn" id="awPrev" type="button">AtrÃ¡s</button>
              <button class="btn btn-primary" id="awNext" type="button">Siguiente</button>
            </div>
          </div>
  
          ${rules.length ? `<div class="small" style="margin-top:8px; color:#7a5200;">${rules.map(r=>`â€¢ ${esc(r.message)}`).join('<br/>')}</div>` : ''}
          ${missingDocs.length ? `<div class=\"small\" style=\"margin-top:8px;\"><strong>Docs faltantes (este paso)</strong><br/>${missingDocs.map(d=>`â€¢ ${esc(d)}`).join('<br/>')}<br/><button class=\"btn\" type=\"button\" onclick=\"location.hash='#/peticiones?petition_id=${petitionId}'\">Ir a Docs</button></div>` : ''}
  ...
  function docsForStep(petitionType, allDocs, stepKey) {
    ...
    // HeurÃ­stica: suficiente para UX (la fuente de verdad sigue siendo checklist/schema).
    if (tU === 'ALTA_FACTURA') {
