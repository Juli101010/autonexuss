 ...
 * Laboral service.
 *
 * Today: manual handling.
 * Future: call DelSol laboral via DelSolClient and store delsol_laboral_id + sync status.
 */
...
async function prepareLaboralCase(_petitionId) {
  const client = clientFromEnv();
  if (!client.isEnabled()) {
    return { enabled: false, message: 'DelSol not enabled' };
  }
  // When enabled: build payload from petition.data and call client.createLaboralCase()
  return { enabled: true, message: 'Not implemented' };
}

module.exports = { prepareLaboralCase, markSyncError };
