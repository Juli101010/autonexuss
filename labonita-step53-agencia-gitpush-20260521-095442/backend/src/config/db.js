...
async function migrate() {
  ...
  await ensureColumn('users', 'onboarding_profile', `ALTER TABLE users ADD COLUMN onboarding_profile TEXT NOT NULL DEFAULT 'ASOCIADA'`);
  await ensureColumn('users', 'onboarding_status', `ALTER TABLE users ADD COLUMN onboarding_status TEXT NOT NULL DEFAULT 'PENDIENTE_VALIDACION'`);
...
`);
  ...
  // NominaSOL worker registry: avoids duplicate worker creation across retries and profile types.
  await run(`
    CREATE TABLE IF NOT EXISTS nominasol_worker_registry (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  `);
  await ensureIndex(`CREATE INDEX IF NOT EXISTS idx_nominasol_worker_registry_tax_profile ON nominasol_worker_registry(tax_id, profile_type)`);
...
async function upgrade() {
  ...
  await ensureColumn('delsol_sync_queue', 'external_code', `ALTER TABLE delsol_sync_queue ADD COLUMN external_code TEXT`);
  await ensureColumn('delsol_sync_queue', 'result_json', `ALTER TABLE delsol_sync_queue ADD COLUMN result_json TEXT`);
