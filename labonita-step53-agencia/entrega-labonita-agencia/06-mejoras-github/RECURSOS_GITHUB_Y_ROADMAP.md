...
- Se instalo `helmet` para headers HTTP seguros en Express.
- Se instalo `express-rate-limit` para limitar intentos de login.
- Se agrego smoke test local con `npm test`.
- Se mantiene `pdf-lib`, Playwright, BullMQ y CASL como siguientes pasos tecnicos, no instalados todavia para no cambiar arquitectura sin cerrar el flujo actual.
