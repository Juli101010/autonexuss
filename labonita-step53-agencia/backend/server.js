...
const { createBackup } = require('./src/modules/backups/backups.service');
...
async function start() {
  ...
  const registerLimiter = rateLimit({
    ...
    standardHeaders: true,
    legacyHeaders: false,
    message: { error: 'Demasiados registros. Probá de nuevo más tarde.' },
  });
  const sensitiveLimiter = rateLimit({
    ...
    standardHeaders: true,
    legacyHeaders: false,
    message: { error: 'Demasiadas operaciones sensibles. Probá de nuevo en unos minutos.' },
  });
  ...
  app.use(cors({
    origin(origin, cb) {
      ...
      if (!allowedOrigins.length) return cb(null, process.env.NODE_ENV !== 'production');
...
app.use('/billing', requirePageRole('ADMIN'), express.static(path.join(publicDir, 'billing')));
  ...
  app.use('/api/admin/delsol', sensitiveLimiter);
  app.get('/api/health', (_req, res) => res.json({ ok: true, service: 'labonita', time: new Date().toISOString() }));
  ...
  const delsolAutoEnabled = String(process.env.DELSOL_AUTO_SYNC_ENABLED || '0') === '1';
  ...
  // ---- Daily SQLite backup (single rotating file) ----
  // Default: every day at 02:00 server time. Disable with BACKUP_AUTO_ENABLED=0.
  const backupAutoEnabled = String(process.env.BACKUP_AUTO_ENABLED || '1') === '1';
  const backupCron = process.env.BACKUP_CRON || '0 2 * * *';
  if (backupAutoEnabled) {
    cron.schedule(backupCron, async () => {
      try {
        const out = await createBackup({ reason: 'cron' });
        // eslint-disable-next-line no-console
        console.log('[backups] created', out.filename, out.size_bytes);
      } catch (err) {
        // eslint-disable-next-line no-console
        console.error('[backups] failed', err.message);
      }
    });
  }
