...
async function monthlyPayroll(req, res) {
  ...
  const parityRows = buildControlAltasMesParityRows(rows);

  if (wantsDownload(req)) return sendDownload(res, parityRows, 'control_altas_mes', downloadFormat(req));
  return res.json({ rows, parity_rows: parityRows });
}
...
async function gastos(req, res) {
  ...
  const parityRows = buildControlGastosParityRows(rows);

  if (wantsDownload(req)) return sendDownload(res, parityRows, 'control_gastos', downloadFormat(req));
  return res.json({ rows, parity_rows: parityRows });
}
...
async function controlTotal(req, res) {
  ...
  const parityRows = buildControlSoloParityRows(rows);
  const gastosRows = buildGastosRows(petitions, docs);
  const monthlyRows = buildMonthlyPayrollRows(petitions, docs);
  const bbddRows = buildBbddAltasParityRows(petitions);
  const validations = buildCrossValidations({
    controlTotalRows: rows,
    gastosRows,
    monthlyPayrollRows: monthlyRows,
    bbddAltasRows: bbddRows,
  });

  if (wantsDownload(req)) return sendDownload(res, parityRows, 'control_solo', downloadFormat(req));
  return res.json({ rows, parity_rows: parityRows, validations });
}
...
async function ssDiario(req, res) {
  ...
  const parityRows = buildBbddAltasParityRows(petitions);

  if (wantsDownload(req)) return sendDownload(res, parityRows, 'bbdd_altas', downloadFormat(req));
  return res.json({ rows, parity_rows: parityRows });
}
