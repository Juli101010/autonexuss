...
const {
  ...
  safeJsonParse,
} = require('../controls/controls.service');
...
const FIELD_LABELS = {
  petition_date: 'Fecha petición', responsible_name: 'Nombre de la asociada/responsable', company_show: 'Cía + espectáculo', reference: 'Referencia', pais_ciudad_actuacion: 'País y ciudad de actuación', fecha_inicio_actuacion: 'Fecha inicio actuación',
...
function profileTypeFromPetitionType(type) {
  const t = String(type || '').toUpperCase();
  if (t === 'PERSONA_ASOCIADA' || t === 'ONBOARDING_ASSOCIADO') return 'ASOCIADA';
  return 'PUNTUAL';
}
...
async function readPetitionsParsed(limit = 2000) {
  const rows = await db.all(
    `SELECT id, user_id, type, status, data, created_at FROM petitions ORDER BY id DESC LIMIT ?`,
    [limit]
  );
  return rows.map((r) => ({ ...r, data: safeJsonParse(r.data, {}) }));
}

async function readDocumentsForControls() {
...
async function readCommissionsByPetition() {
  const rows = await db.all('SELECT petition_id, amount_eur FROM commissions ORDER BY id DESC');
  const totals = new Map();
  rows.forEach((r) => {
    const petitionId = Number(r.petition_id);
    totals.set(petitionId, (totals.get(petitionId) || 0) + Number(r.amount_eur || 0));
  });
  return totals;
}
...
async function readLedgerTotalsByPetition() {
  ...
  const totals = new Map();
  rows.forEach((r) => {
    totals.set(Number(r.petition_id), {
...
async function createQueue({ entity_type, entity_id, petition_id, operation, payload }) {
  const existing = await db.get(
    `SELECT id FROM delsol_sync_queue
     WHERE entity_type = ? AND entity_id = ? AND operation = ? AND status IN ('pending','processing','synced','blocked_permission')
...
exports.createLaboralMovement = async (req, res) => {
  try {
    ...
    const payload = {
      ...
      profile_type: profileTypeFromPetitionType(p.type),
...
exports.createInvoice = async (req, res) => {
  try {
    ...
    const existingInvoice = await db.get(
      `SELECT id, internal_number, total, net_artist, commission_amount
       FROM lb_internal_invoices
       WHERE petition_id = ? AND status <> 'ANULADA'
       ORDER BY id DESC LIMIT 1`,
      [id]
    );
    if (existingInvoice) {
      await createQueue({
        entity_type: 'invoice',
        entity_id: existingInvoice.id,
        petition_id: id,
        operation: 'create_invoice_draft',
        payload: { existing: true, internal_number: existingInvoice.internal_number },
      });
      return res.json({
        ok: true,
        id: existingInvoice.id,
        internal_number: existingInvoice.internal_number,
        total: money(existingInvoice.total),
        net_artist: money(existingInvoice.net_artist),
        commission_amount: money(existingInvoice.commission_amount),
        existing: true,
      });
    }
...
exports.createLiquidation = async (req, res) => {
  try {
    ...
    await createQueue({
      ...
      entity_id: r.lastID,
      petition_id: id,
      operation: 'create_laboral_or_payment',
      payload: { ejercicio: '2026', gross, commission, irpf, net, person, profile_type: profileTypeFromPetitionType(p.type) },
    });
...
exports.exportCsv = async (req, res) => {
  try {
    ...
    for (const p of rows) {
      const data = safeJson(p.data);
      const c = await controlForPetition(p.id);
      const base = Number(c.income_total || 0);
      const gastos = Number(c.expenses_total || 0);
      const total = base;
      const comisionBonita = Math.max(0, Number(base - c.liquidations_total - gastos || 0));
      parityRows.push({
        'REFERENCIA': referenceFromData(p),
        'SITUACION': p.status,
        'Nº FACTURA': '',
        'TOTAL': total,
        'IVA': 0,
        'BASE': base,
        'COMISION LA BONITA': comisionBonita,
        'COMISION SERVICIOS EXTRAS': 0,
        'RESTO': total - comisionBonita,
        'SEG SOCIAL + IRPF': 0,
        'TOTAL A DEPOSITAR': total - comisionBonita,
        'LIQUIDADO   (FECHA)': '',
        'GASTOS PUNTUALES': 0,
        'GASTOS GENERALES': gastos,
        'A CALCULAR EN NOMINA ( COSTE TOTAL)': gastos,
        'ALTA/BAJA': '',
        'TRAMO O CONTRATO': '',
        'LIQUIDO          ( EN MANO)': total - comisionBonita,
        'SSOCIAL': 0,
        'IRPF': 0,
      });
    }
    const headers = Object.keys(parityRows[0] || { REFERENCIA: '' });
    const lines = [headers.join(';')];
    parityRows.forEach((row) => {
      lines.push(headers.map((h) => `"${String(row[h] ?? '').replace(/"/g, '""')}"`).join(';'));
    });
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename="control-solo.csv"');
    res.send('\ufeff' + lines.join('\n'));
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.exportAltasBajasCsv = async (_req, res) => {
  try {
    ...
    const header = ['A1','ANOTACIONES','TRABAJADOR','ALTA','BAJA','INFO NOMINA','REFERENCIA','SOCI','FACTURA','COMISION 6%','COMISION EXTRA','GASTOS PUNTUALES','FRAS PUNTUALES ','TOTAL GASTOS','COSTE TOTAL'];
    ...
    for (const r of rows) {
      lines.push([
        ...
        r.end_date || '',
        r.movement_type || '',
        referenceFromData({ ...r, id: r.petition_id, data: r.data }),
        r.type === 'PERSONA_ASOCIADA' ? 'SI' : '',
        '',
        0,
        0,
        0,
    ...
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename="control-altas-mes.csv"');
    res.send('\ufeff' + lines.join('\n'));
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.exportGastosCsv = async (_req, res) => {
  try {
    ...
    const header = ['MES','REFERENCIA','OBSERVACIONES','TARIFA EXTRA','AVISA','DIF-PUNT','FACTURAS','TICKETS','TOTAL','COMPENSADO','TOTAL_COMPENSACION','ENTREGADOS','TOTAL_GASTOS_GENERALES','RESTO'];
    const lines = [header.join(';')];
    for (const r of rows) {
      lines.push([
        String(r.occurred_at || '').slice(0, 7),
        referenceFromData({ ...r, id: r.petition_id, data: r.data }),
        r.label || '',
        0,
        '',
    ...
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename="control-gastos.csv"');
    res.send('\ufeff' + lines.join('\n'));
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
// Override legacy CSV exports with parity-driven builders from controls module.
exports.exportCsv = async (_req, res) => {
  try {
    const petitions = await readPetitionsParsed(2000);
    const commissionsByPetition = await readCommissionsByPetition();
    const ledgerTotalsByPetition = await readLedgerTotalsByPetition();
    const controlRows = buildControlTotalRows(petitions, commissionsByPetition, ledgerTotalsByPetition);
    const parityRows = buildControlSoloParityRows(controlRows).map((row, idx) => {
      const source = controlRows[idx] || {};
      return {
        PETICION_ID: source.petition_id ?? '',
        ...row,
      };
    });
    ...
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename="control-solo.csv"');
    res.send(csvFromRows(parityRows, ['PETICION_ID', 'REFERENCIA']));
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
...
exports.exportAltasBajasCsv = async (_req, res) => {
  try {
    const petitions = await readPetitionsParsed(2000);
    const monthlyRows = buildMonthlyPayrollRows(petitions);
    const parityRows = buildControlAltasMesParityRows(monthlyRows).map((row, idx) => ({
      PETICION_ID: monthlyRows[idx]?.petition_id ?? '',
      ...row,
    }));

    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename="control-altas-mes.csv"');
    res.send(csvFromRows(parityRows, ['PETICION_ID', 'A1']));
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
...
exports.exportGastosCsv = async (_req, res) => {
  try {
    const petitions = await readPetitionsParsed(2000);
    const docs = await readDocumentsForControls();
    const gastosRows = buildGastosRows(petitions, docs);
    const parityRows = buildControlGastosParityRows(gastosRows);

    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename="control-gastos.csv"');
    res.send(csvFromRows(parityRows, ['MES']));
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
