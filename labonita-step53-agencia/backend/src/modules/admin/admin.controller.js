...
exports.requestMoreInfo = async (req, res) => {
  try {
    ...
    if (!petition) return res.status(404).json({ error: 'PeticiÃ³n no encontrada' });
    ...
    const finalNote = note || 'Faltan datos, por favor revisÃ¡ el checklist y completÃ¡ lo faltante.';
    ...
    const msg = summary
      ? `Admin solicita mÃ¡s informaciÃ³n en tu peticiÃ³n #${id}. ${finalNote} | ${summary}`
      : `Admin solicita mÃ¡s informaciÃ³n en tu peticiÃ³n #${id}. ${finalNote}`;
...
exports.approvePetition = async (req, res) => {
  try {
    ...
    const petition = await db.get(`SELECT * FROM petitions WHERE id = ?`, [id]);
    if (!petition) return res.status(404).json({ error: 'Peticion no encontrada' });
    ...
    await db.transaction(async () => {
      if (isOnboarding) {
        ...
        if (!socioFields.artist_name || !socioFields.email) {
          throw new Error('Datos obligatorios incompletos para crear socio (nombre artÃ­stico + email)');
        }
      ...
      const msg =
        isOnboarding
          ? (membershipType === 'PUNTUAL'
              ? 'Tu solicitud fue APROBADA. Ya estÃ¡s habilitado como PUNTUAL.'
              : 'Tu solicitud fue APROBADA. Ya sos socio.')
          : `Tu peticiÃ³n #${id} fue APROBADA.`;
...
// Auto-facturaciÃ³n (no rompe aprobaciÃ³n si billing falla)
try {
...
exports.rejectPetition = async (req, res) => {
  try {
    ...
    const fromStatus = petition.status;
    const finalReason = reason || 'La administracion rechazo la peticion por incompatibilidad con el flujo operativo.';
    ...
    await notifyOnce(
      petition.user_id,
      `Tu solicitud fue RECHAZADA. Motivo: ${finalReason}`,
...
exports.deactivateSocio = async (req, res) => {
  try {
    ...
    await db.transaction(async () => {
      ...
      await notifyOnce(
        socio.user_id,
        'Tu membresÃ­a fue desactivada por administraciÃ³n.',
...
exports.remindPetition = async (req, res) => {
  try {
    ...
    const msg = summary
      ? `Recordatorio de administraciÃ³n: tu peticiÃ³n #${id} sigue incompleta (${petition.status}). ${petition.admin_note || ''} | ${summary}`.trim()
      : `Recordatorio de administraciÃ³n: tu peticiÃ³n #${id} sigue incompleta (${petition.status}). ${petition.admin_note || ''}`.trim();
...
exports.updatePetitionStatus = async (req, res) => {
  try {
    ...
    if (!allowed.includes(to_status)) {
      return res.status(409).json({ error: `TransiciÃ³n no permitida: ${from} -> ${to_status}` });
    }
...
exports.assignPetition = async (req, res) => {
  ...
  if (!p) return res.status(404).json({ error: 'PeticiÃ³n no encontrada' });
...
exports.getPetitionSchema = async (req, res) => {
  ...
  if (!row) return res.status(404).json({ error: 'PeticiÃ³n no encontrada' });
