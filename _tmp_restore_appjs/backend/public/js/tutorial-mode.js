(function () {
  ...
  const nativeAdminSteps = [
    { hash: "#/dashboard", sel: ".brand", title: "Prueba integral real", body: "Crea un circuito completo de prueba: observada, rechazada y completada con laboral, factura interna, liquidacion y cola DelSol.", requireClick: false, demoAction: true },
  ...
  function render() {
    ...
    card.innerHTML = `
      ...
      ${step.demoAction ? `
        <div class="tutorial-demo-box">
          <button type="button" class="primary" data-act="demo-run">Crear prueba integral real</button>
          <button type="button" data-act="demo-sync" disabled>Procesar DelSol de la prueba</button>
          <div class="tutorial-demo-out" data-demo-out>Primero crea datos reales: observada, rechazada y completa con laboral, factura interna, liquidacion y cola DelSol pendiente.</div>
        </div>
      ` : ""}
    ...
    const onDemoRun = card.querySelector('[data-act="demo-run"]');
    const onDemoSync = card.querySelector('[data-act="demo-sync"]');
    ...
    if (onDemoRun) onDemoRun.onclick = () => runIntegralDemo(onDemoRun);
    if (onDemoSync) onDemoSync.onclick = () => syncIntegralDemo(onDemoSync);
    updateActionState();
  }

  async function runIntegralDemo(button) {
    const out = card?.querySelector("[data-demo-out]");
    button.disabled = true;
    if (out) out.textContent = "Creando peticiones, factura, laboral, liquidacion y cola DelSol pendiente...";
    try {
      const res = await fetch("/api/admin/demo/integral", {
        method: "POST",
        credentials: "include",
      ...
      if (!res.ok) throw new Error(data.error || "No se pudo crear la prueba integral");
      window.__LaBonitaDemoResult = data;
      const completed = data?.scenarios?.completed;
      const observed = data?.scenarios?.observed;
      const rejected = data?.scenarios?.rejected;
      if (out) {
        out.innerHTML = `
          <strong>Prueba creada.</strong><br>
          Observada: #${observed?.id || "-"} (${observed?.status || "-"})<br>
          Rechazada: #${rejected?.id || "-"} (${rejected?.status || "-"})<br>
          Completa: #${completed?.id || "-"} (${completed?.status || "-"})<br>
          Cola DelSol: ${(completed?.queue || []).map((q) => `#${q.id} ${q.entity_type} ${q.status}`).join(", ") || "sin items"}
        `;
      }
      const syncBtn = card?.querySelector('[data-act="demo-sync"]');
      if (syncBtn) syncBtn.disabled = false;
      stepDone = true;
      updateActionState();
      if (completed?.id) {
        setTimeout(() => {
          location.hash = `#/peticiones?petition_id=${completed.id}`;
        }, 900);
      }
    } catch (e) {
  ...
  async function syncIntegralDemo(button) {
    const out = card?.querySelector("[data-demo-out]");
    button.disabled = true;
    if (out) out.textContent = "Procesando cola DelSol real de la prueba...";
    try {
      const res = await fetch("/api/admin/delsol/queue/process", {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error || "No se pudo procesar DelSol");
      if (out) out.innerHTML = `<strong>DelSol procesado.</strong><br>Items procesados: ${data.processed || 0}. Revisa DelSol Sync para ver synced/error por item.`;
      setTimeout(() => { location.hash = "#/delsol"; }, 900);
    } catch (e) {
      if (out) out.textContent = `Error DelSol: ${e.message}`;
      button.disabled = false;
    }
  }
